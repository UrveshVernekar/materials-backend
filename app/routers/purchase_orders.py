...
def list_purchase_orders(material_code: str = Query(..., description="Material code to fetch purchase orders for"), db: Session = Depends(get_db)):
...
def create_purchase_order_endpoint(po_in: PurchaseOrderCreate, db: Session = Depends(get_db)):
    po, error = create_purchase_order(db, po_in)
    if error:
        raise HTTPException(status_code=400, detail=error)
    return po

@router.put("/{po_id}", response_model=PurchaseOrderResponse)
def update_purchase_order_endpoint(po_id: int, po_in: PurchaseOrderUpdate, db: Session = Depends(get_db)):
